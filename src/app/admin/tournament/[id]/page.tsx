...
export default function TournamentDetailPage({ params }: { params: Promise<{ id: string }> }) {
    ...
    // Scoreboard History Handling
    useEffect(() => {
        if (isScoreboardOpen) {
            // Push state when opened
            window.history.pushState({ scoreboard: true }, "", window.location.pathname + "#scoreboard");
                                    ...
                                    <div className="flex items-center gap-2 w-full md:w-auto overflow-x-auto no-scrollbar">
                                        ...
                                        <button
                                            onClick={() => {
                                                ...
                                                toast.info("คำแนะนำ: เอียงมือถือเป็นแนวนอน เพื่อการใช้งานที่สะดวกที่สุด", {
                                                    duration: 5000,
                                                    icon: <MonitorPlay className="h-4 w-4" />
                                                });
                                            }}
                                            className="flex-1 md:flex-none flex items-center justify-center gap-2 px-3 py-1.5 rounded-lg text-xs font-bold transition-all border whitespace-nowrap bg-purple-500/20 text-purple-400 border-purple-500/30 hover:bg-purple-500/30"
                                        >
